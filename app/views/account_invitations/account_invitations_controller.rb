class AccountInvitationsController < ApplicationController
  ...
  before_action :set_account_invitation, only: %i[destroy update]
  ...
  def destroy
    ...
    flash.now[:notice] = t('shared.destroy.destroyed_m', model: AccountInvitation.model_name.human)
  end
  ...
  def account_invitation_params
    params.require(:account_invitation).permit(:email, :name)
  end
