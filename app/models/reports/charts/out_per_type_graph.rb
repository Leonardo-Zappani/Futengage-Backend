module Reports
  module Charts
    class OutPerTypeGraph < ApplicationInteractor
      def call
        ...
        transactions = transactions.where.not("transaction_type_cd = 0")
      ...
      def create_response(transactions:)
        ...
        transactions.transaction_types.each do |type|

          result << [I18n.t("enums.transaction_type.#{type[0]}"), transactions.where(transaction_type_cd: type[1]).sum(:exchanged_amount_cents).to_f / 100] unless transactions.where(transaction_type_cd: type[1]).sum(:exchanged_amount_cents).zero?
