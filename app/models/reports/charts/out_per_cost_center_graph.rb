module Reports
  module Charts
    class OutPerCostCenterGraph < ApplicationInteractor
      def call
      ...
      def create_response(transactions:)
        result = []
        report = context.context
        context.context.account.cost_centers.each do |cost_center|
          result << [cost_center.name, transactions.where(cost_center: cost_center).sum(:exchanged_amount_cents).to_f / 100] unless transactions.where(cost_center: cost_center).sum(:exchanged_amount_cents).zero?
